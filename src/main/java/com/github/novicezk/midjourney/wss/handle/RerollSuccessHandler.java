...
public class RerollSuccessHandler extends MessageHandler {
	private static final String CONTENT_REGEX_1 = "\\*\\*(.*)\\*\\* - <@\\d+>(?:\\s+\\[.*?\\]\\(<.*?>\\))? \\((.*?)\\)";
	private static final String CONTENT_REGEX_2 = "\\*\\*(.*)\\*\\* - Variations by <@\\d+>(?:\\s+\\[.*?\\]\\(<.*?>\\))? \\((.*?)\\)";
	private static final String CONTENT_REGEX_3 = "\\*\\*(.*)\\*\\* - Variations \\(.*?\\) by <@\\d+>(?:\\s+\\[.*?\\]\\(<.*?>\\))? \\((.*?)\\)";
